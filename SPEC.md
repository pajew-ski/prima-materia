...
**Ordnerregel, jetzt maschinell geprüft.** `tests/test_data_dir_registration.py` vergleicht die fünf Listen gegen den Dateibaum und meldet jeden Ordner mit `.ttl`-Dateien, der in einer davon fehlt. Der Vergleich läuft ausdrücklich gegen den Dateibaum und nicht gegen eine sechste Liste; eine sechste Liste wäre dieselbe Fehlerklasse eine Ebene höher. Ausgenommen sind `shapes/` und `build/`, und wer dort etwas einträgt, nimmt einen Ordner aus der Prüfung und muss es begründen.

Die Regel selbst bleibt zu kennen, denn der Wächter sagt, dass etwas fehlt, und nicht, was einzutragen ist. Ein neuer Datenordner muss an fünf Stellen nachgezogen werden, sonst wird er still nicht kompiliert und nicht validiert: `DEFAULT_INPUTS` in `scripts/compile.py`, `DEFAULT_INPUTS` in `scripts/publish.py`, `DEFAULT_DATA_DIRS` in `scripts/validate.py`, `SCAN_DIRS` in `tests/test_no_substance_classes.py`, `SCAN_DIRS` in `tests/test_identifier_uniqueness.py`. Nichts schlägt fehl, wenn eine davon vergessen wird. Die fünfte ist die teuerste: ohne sie prüft der Kollisionswächter den neuen Ordner nicht, zwei Knoten mit demselben Bezeichner verschmelzen beim Kompilieren zu einem, und jede SHACL-Bedingung ist danach doppelt erfüllt.
...
**Eine offene Frage, die aus dem Zustand des Bestands folgt, wird nicht eingetragen. Sie wird abgefragt.** Der Bestand hält dafür bereits die Zustände, und dieses Dokument leitet Warteschlangen ausdrücklich aus ihnen ab statt sie zu führen: die dünn erschlossenen Traditionen aus `pm:coverageState pm:corpusNamed` (§14), die ungeöffneten Werke erschlossener Traditionen aus dem Fehlen eines `pm:fromWork` (§14) und die ungeprüften Verallgemeinerungen aus dem Fehlen eines `pm:Testing` (§16). Jedes Mal steht dort derselbe Satz — ein zweites Verzeichnis entsteht nicht. Er gilt für den Tracker so gut wie für eine Datei.
...
**Die Grenze dieser Regel lag dort, wo der Zustand nicht im Graphen stand, und die größte davon ist geschlossen.** Dass eine Überlieferung für eine bestimmte Frage befragt wurde und schwieg, stand bis zum 2026-09-09 ausschließlich in den kumulierenden `korpus:`-Labels; im Graphen war ein nie befragtes Werk von einem befragten und stummen nicht zu unterscheiden. Das ist derselbe Unterschied, den `pm:noProcedureDevised` gegenüber `pm:casesWithoutDeviation` sichert und den die Sachachsen mit ihrem nicht-angegeben-Wert sichern, nur eine Ebene höher.

**`pm:Searching` trägt ihn jetzt.** Ein Suchknoten nennt den Korpus, die Frage, die tatsächlich benutzten Begriffe samt Transliterationen, den Ausgang, den Suchenden und das Datum. Vier Ausgänge: nichts gefunden, Stelle gefunden, Gegenmaterial gefunden, Korpus nicht erreichbar. Der letzte ist der Grund, warum die Skala kein Ja-Nein ist — ein unerreichbarer Korpus und ein gelesener stummer sind entgegengesetzte Befunde, und ohne eigenen Wert würden beide als der zweite eingetragen.

**Damit ist die Abdeckung abfragbar und der Tracker entlastet:** ein Behauptungs-Issue muss nicht mehr offen bleiben, nur weil es seine eigene Suchabdeckung trägt. Was noch nicht befragt wurde, folgt aus den Suchknoten, die es nicht gibt.

**Der zweite Ertrag ist der wichtigere und war nicht der Anlass.** Eine Suche ohne `pm:searchYieldedNode` ist eine Lücke, die der Bestand über sich selbst ausweist: hier wurde gefragt, hier steht nichts, und hier könnte jemand ansetzen, der die Ausgabe besitzt. Das ist ein beseitigtes Tor im Sinne von §0 und keine Buchführung — eine ausgestellte Lücke kann ein Fremder schließen, eine unausgestellte nicht. Der Bestand hört damit auf, nur zu zeigen, was er weiß, und zeigt auch, wo er nachgesehen hat und leer ausging.

**Was der Knoten nicht ist.** Er ist kein Frageknoten. Er wird geschrieben, wenn jemand die Suche geführt hat, nie im Voraus; sein Fehlen bleibt aussagekräftig, wie beim Prüfknoten, und ein Stub für jede ungestellte Frage würde genau die Unterscheidung zerstören, für die er gebaut ist. Der Rückverweis von einem Knoten auf sein Issue steht in einer `skos:note` in der Form `prima-materia#42`, niemals in `dcterms:source`: ein Issue ist kein Werk, und `pm:SourceIsLiteratureShape` weist die URL ohnehin ab.
...
**Die Gegensuche läuft hier zuerst gegen den eigenen Bestand, und sie läuft, bevor die Verallgemeinerung geschrieben wird.** §15 verlangt die Suche nach der Negation jeder Behauptung, und eine Verallgemeinerung ist eine. Der Unterschied zum Traditionsknoten ist der Preis: sie behauptet etwas über eine Art von Überlieferung, und der Bestand hält bereits Dutzende in derselben Klasse, so dass ein Gegenfall typischerweise schon dasteht und eine Abfrage kostet statt einer Recherche.

**Der Fehler, gegen den das steht, ist am 2026-09-09 belegt und war teuer.** Vier Verallgemeinerungen wurden aus der Kautelenschicht gebildet, ohne im Bestand nach ihrem Gegenteil zu suchen; die erste danach geführte Prüfung widerlegte die erste von ihnen aus einem Knoten, der seit Wochen dastand. Die Verwechslung dahinter ist die, die den Schritt so leicht ausfallen lässt: wer aus dem Bestand verallgemeinert, hat gerade die Klasse gelesen, aus der die Belege stammen, und nicht die, in der der Gegenfall liegt. Siehe prima-materia#667.
...
Ein dritter Wert ist nicht geschuldet, sondern wird geschrieben, wenn ein Fall ihn verlangt: die ausdrückliche Ungebundenheit, für eine Quelle, die die Unterscheidung anerkennt und die Praxis über sie hinweg vorschreibt. `pm:anchoringUnbound` steht für das fortwährende Gebet und trägt genau das; die übrigen Achsen haben keinen solchen Wert, weil im Bestand kein Fall dafür vorliegt. Siehe prima-materia#440.
