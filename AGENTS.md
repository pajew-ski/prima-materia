...
**Die Prüfung liegt vor dem PR, nicht danach.** Auf einem Arbeitsbranch entsteht der Lauf erst, wenn `prima_repo_check` ihn anstößt: das Werkzeug löst `validate.yml` per `workflow_dispatch` aus, wenn es für den Kopf-SHA keinen Lauf mit Befund gibt, und meldet dann `laeuft`. Also: alle Schreibvorgänge des Bündels, dann `prima_repo_check`, nach etwa einer Minute erneut, und erst bei `gruen` den PR öffnen. Zwischencommits erzeugen keinen Lauf; das ist gewollt, weil sie planmäßig rot sein dürfen und jeder rote Lauf eine Benachrichtigung auslöste (pajew-ski/data#337). `prima_repo_check` nach jedem einzelnen Schreibvorgang aufzurufen, stößt entsprechend für jeden Zwischenstand einen Lauf an und ist zu unterlassen.
