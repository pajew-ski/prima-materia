...
- **Änderungen an `.github/workflows/*`.** Das Agenten-Token trägt keinen `workflows`-Scope; jeder Schreibversuch endet mit `403`. Der früher hier vermerkte Engpass ist erledigt: `validate.yml` hört inzwischen auf `push: branches: [main, 'claude/**']` mit `cancel-in-progress`, also läuft auf einem Arbeitsbranch geprüft, bevor ein PR offen ist, und `prima_repo_check` liefert dort einen echten Stand statt `kein_lauf`. Zwei Patches liegen fertig vor und warten auf das Einspielen von Hand: prima-materia#352 (`validate.yml` soll beim bloßen Anlegen eines Branches nicht laufen) und prima-materia#353 (`distribute.yml` braucht eine serialisierende `concurrency`-Gruppe). Beide sind in `SPEC.md` §7 als Soll nachgetragen.
